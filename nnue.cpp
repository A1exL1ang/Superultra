...
Score neuralNetwork::eval(Color col, int8 pieceCount){
	...
	for (int i = 0; i < 8; i++) {
        eval += mm256_extract_epi32_var_indx(vectorEval, i);
    }
...
void initNNUEWeights(){
    ...
    // W1
    std::memcpy(W1, gnnueNetData + idx, inputHalf * hiddenHalf * sizeof(NNUEWeight));
    idx += inputHalf * hiddenHalf * sizeof(NNUEWeight);

    // B1
    std::memcpy(B1, gnnueNetData + idx, hiddenHalf * sizeof(NNUEWeight));
    idx += hiddenHalf * sizeof(NNUEWeight);

    // W2
    std::memcpy(W2, gnnueNetData + idx, outputWeightBucketCount * hiddenHalf * 2 * sizeof(NNUEWeight));
    idx += hiddenHalf * sizeof(NNUEWeight) * 2;
    
    // B2
    std::memcpy(B2, gnnueNetData + idx, outputWeightBucketCount * sizeof(NNUEWeight));
}
